...
func (repository *RekapServiceImpl) UpdateRekapPesananBiaya(ctx context.Context, id int, id_pesanan []string, id_biaya []string) error {
	tx, err := repository.DB.Begin()
	defer helperMain.ErrorTx(tx)
	helperMain.PanicIfError(err)
	var pesanan []int
	...
	err = repository.RepositoryRekap.UpdateIdRekapPesananBiaya(ctx, tx, id, pesanan, biaya)
	return err
}
